...
app.get('/image', function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var qName, qWidth, qHeight, height, width, listImg, _a, response, reject;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                qName = req.query.name;
                qWidth = req.query.width;
                qHeight = req.query.height;
                height = parseInt(qHeight);
                width = parseInt(qWidth);
                listImg = ['img1', 'img2', 'img3'];
                if (!qName || !width || !height || +width <= 0 || +height <= 0) {
                    return [2 /*return*/, res.status(400).json({ msg: " input wrong" })];
                }
                return [4 /*yield*/, (0, resizeImage_1.default)(qName, width, height)];
            case 1:
                _a = _b.sent(), response = _a.response, reject = _a.reject;
                if (!listImg.includes(qName))
                    return [2 /*return*/, res.status(400).json({ msg: "the name of image is wrong" })];
                return [2 /*return*/, res.sendFile(response)];
        }
    });
}); });
